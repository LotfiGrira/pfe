...
const Show: React.FC<Props> = ({ examen, question }) => {
  ...
  const handlePropositionChange = (index: number, field: keyof Proposition, value: any) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    put(route('question.update', {
      examen: examen.id,
      id: question.id
    }), {
      onSuccess: () => {
        router.visit(route('question.affichage', { examen: examen.id }));
      }
    });
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="block font-medium text-gray-700 mb-1">Titre de la question</label>
          <input
            type="text"
            value={data.titre}
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-lg font-semibold text-gray-700">Propositions</h2>
            <button
              type="button"
          ...
          {errors.propositions && (
            <p className="mb-2 text-red-500 text-sm">{errors.propositions}</p>
          )}
          ...
          {data.propositions.map((prop, index) => (
            <div key={index} className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
              ...
              {errors[`propositions.${index}.propos`] && (
                <p className="text-red-500 text-sm mb-2">{errors[`propositions.${index}.propos`]}</p>
              )}
